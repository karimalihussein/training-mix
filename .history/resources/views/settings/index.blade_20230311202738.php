                ...
                <div class="p-6 bg-white border-b border-gray-200">
                   <form action="{{ route('settings.update') }}" method="POST">
                    ...
                    <div class="mt-4">
                        ...
                        <select name="supported_languages[]" id="supported_languages" class="block mt-1 w-full js-example-basic-multiple" multiple>
                            @foreach (config('available-locales') as $key => $locale)
                                <option value="{{ $key  }}" 
                                    @if (in_array($key, $values['supported_languages']))
                                        selected
                                    @endif
                                >
                                    {{ $locale['name']  }}
